...
import brochuresCatalog from "@/lib/config/brochuresAndBooklets.json";
...
const DEFAULT_ARTICLE_SERIES: StoredReadingVideo[] = (articleSeriesCatalog as CatalogReadingVideo[])
  .map((item) => ({
    id: item.id,
    title: item.title,
...
const DEFAULT_TRACTS: StoredReadingVideo[] = (tractsCatalog as CatalogReadingVideo[])
  .map((item) => ({
    id: item.id,
    title: item.title,
    duration_seconds: Math.max(0, Math.trunc(Number(item.duration_seconds) || 0)),
...
const DEFAULT_BROCHURES: StoredReadingVideo[] = (brochuresCatalog as CatalogReadingVideo[])
  .map((item) => ({
    id: item.id,
    title: item.title,
    duration_seconds: Math.max(0, Math.trunc(Number(item.duration_seconds) || 0)),
...
function normalizeState(raw: unknown): StoredReadingState {
  ...
  return {
    ...
    broadcasting_videos: Array.isArray(state.broadcasting_videos)
      ...
      : DEFAULT_BROADCASTING_VIDEOS,
    article_series: Array.isArray(state.article_series)
      ? mergeWithDefaultVideos(
          state.article_series
            .map((video) => normalizeVideo(video, nowIso))
            .filter((video): video is StoredReadingVideo => Boolean(video)),
    ...
    books: Array.isArray(state.books)
      ...
      : DEFAULT_READING_BOOKS,
    tracts: Array.isArray(state.tracts)
      ? mergeWithDefaultVideos(
          state.tracts
            .map((video) => normalizeVideo(video, nowIso))
            .filter((video): video is StoredReadingVideo => Boolean(video)),
    ...
    apostilas: Array.isArray(state.apostilas)
      ...
      : DEFAULT_APOSTILAS,
    brochures: Array.isArray(state.brochures)
      ? mergeWithDefaultVideos(
          state.brochures
            .map((video) => normalizeVideo(video, nowIso))
            .filter((video): video is StoredReadingVideo => Boolean(video)),
...
export async function setArticleSeriesRead(userEmail: string, itemId: string, read: boolean) {
  const state = await getState(userEmail);
  const nowIso = new Date().toISOString();
  const nextItems = state.article_series.map((item) =>
...
export async function setTractRead(userEmail: string, itemId: string, read: boolean) {
  const state = await getState(userEmail);
  const nowIso = new Date().toISOString();
  const nextItems = state.tracts.map((item) =>
    item.id === itemId ? { ...item, read, updated_at: nowIso } : item
  );
...
export async function setBrochureRead(userEmail: string, itemId: string, read: boolean) {
  const state = await getState(userEmail);
  const nowIso = new Date().toISOString();
  const nextItems = state.brochures.map((item) =>
    item.id === itemId ? { ...item, read, updated_at: nowIso } : item
  );
